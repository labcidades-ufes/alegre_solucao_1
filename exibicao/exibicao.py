...
def criar_mapa():
    # Localização inicial no mapa (Alegre, ES)
    localizacao_inicial = [-20.862358636236584, -41.53134351586911]
    zoom_inicial = 10
    ...
    gdf_unidades_saude_alegre = gpd.read_file(caminho_geojson_unidades_saude_alegre)
    ...
    gdf_population_alegre_centroides = gpd.read_file(caminho_geojson_population_alegre_centroides)
    ...
    #ajusta o crs
    gdf_municipio_alegre = gdf_municipio_alegre.to_crs(epsg=4674)
    gdf_unidades_saude_alegre = gdf_unidades_saude_alegre.to_crs(epsg=4674)
    gdf_unidades_saude_apoio = gdf_unidades_saude_apoio.to_crs(epsg=4674)
    gdf_population_alegre = gdf_population_alegre.to_crs(epsg=4674)
    gdf_population_alegre_centroides = gdf_population_alegre_centroides.to_crs(epsg=4674)
    ...
    # Adicionar a camada municipio_alegre
    folium.GeoJson(
        gdf_municipio_alegre.geometry,
        name='Limites da cidade de Alegre',
        style_function=lambda x: {'color': 'green'}
    ).add_to(mapa)
    
    # Adicionar as unidades de saúde
    if not gdf_unidades_saude_alegre.empty:
        for idx, row in gdf_unidades_saude_alegre.iterrows():
    ...
    if not gdf_viario_expandido.empty:
        ...
        folium.GeoJson(
            ...
            name='Sistema Viário Expandido Alegre',
            style_function=lambda x: {'color': 'blue'},
            show=False
        ).add_to(mapa)
    else:
        print("Nenhuma rodovia expandida para exibir em Alegre.")

    # Verificar se há dados de população para exibir
    if not gdf_population_alegre.empty:
        # Adicionar a camada filtrada de população em Alegre
        folium.GeoJson(
            gdf_population_alegre.geometry,
            name='População Alegre',
            style_function=lambda x: {'color': 'red'},
            show=False
        ).add_to(mapa)

    else:
        print("Nenhum dado de hexágonos para exibir em Alegre.")
    ...
    for idx, row in gdf_population_alegre_centroides.iterrows():
        ...
        ui.tags.div(
            ui.tags.h1(
                ...
                ui.tags.span(
                    ...
                    ui.tags.span("| Alegre", style="color: #FEAA01; ; margin: 0; display: inline-block;")                
                ),
