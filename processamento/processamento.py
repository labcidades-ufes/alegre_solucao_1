...
def obter_centroides():
    ...
    #ajusta o crs
    gdf_population_alegre = gdf_population_alegre.to_crs(epsg=31984)
    # Calcular os centroides dos hexágonos
    gdf_population_alegre['centroid'] = gdf_population_alegre.geometry.centroid
    #copia apenas os centroides para o novo geodataframe
    gdf_centroides = gpd.GeoDataFrame(gdf_population_alegre[['centroid']].copy(), geometry='centroid')  # Usar 'geometry' para especificar a coluna de geometria
...
def obter_dados_viarios_expandido(buffer_size=5000):
    ...
    municipio_alegre_buffered = municipio_alegre.buffer(buffer_size)
    municipio_alegre_buffered =  municipio_alegre_buffered.to_crs(epsg=4674)
    
    # Definir um polígono ao redor de Alegre
    polygon = municipio_alegre_buffered.unary_union
...
def obter_rotas_centroide_para_saude():
    ...
    centroides = gpd.read_file(caminho_geojson_population_alegre_centroides)
    unidades_saude = gpd.read_file(caminho_geojson_unidades_saude_alegre)
    ...
    for idx, row in centroides.iterrows():
        ...
        try:
            rota = nx.shortest_path(grafo_viario, nodo_origem, nodo_destino, weight='length')
            #print(f"Rota calculada: {rota}")
            rota_coords = [(grafo_viario.nodes[node]['x'], grafo_viario.nodes[node]['y']) for node in rota]
...
def obter_gdf_peso_hexagonos():
    ...
    # Carregar os dados
    grafo_viario = ox.load_graphml(caminho_grafo)
    gdf_hexagonos = gpd.read_file(caminho_geojson_population_alegre)
    centroides = gpd.read_file(caminho_geojson_population_alegre_centroides)
    unidades_saude = gpd.read_file(caminho_geojson_unidades_saude_alegre)
